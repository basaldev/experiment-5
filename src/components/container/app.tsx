...
import { currentPage, getQuestionnaireFinished, getMessages, getDocuments, getSessionAttributes, getDianosis ,getInputText, getmySuggestions, getQuestionnaire, getScans } from 'domain/store/selectors/main';
...
export function App() {
  const content = (pageName => {
    switch (pageName) {
      case 'HOME_PAGE':
        return  <QuestionnaireView questionnaire={getQuestionnaire()} questionnaireFinished={getQuestionnaireFinished()} scans={getScans()} />;
      case 'SECOND_PAGE':
  ...
  return (
    ...
    `}>
      ...
      <Fab color="primary" aria-label="Brain Activity" className={css`
      && {
      position:absolute;
      bottom: 5vh;
      right: 0;
